...
const AvCategory: React.FC<TAvCategoryProps> = ({category}) => {
  ...
  return <div className={style.avCategory}>
    <div className={style.categoryCode}>{category.code}</div>
    <div className={style.categoryTranslate}>{category[i18n.language as TLng]}</div>
    <div className={style.deleteButton} onClick={deleteThisCategory}><AiOutlineCloseCircle size={20} color="var(--red-bg-color)" /></div>
  </div>;
}
